...
def checksum ( packet_string )
    ...
    csum = 0
    countTo = (len ( packet_string )  2)
    2

    for count in range ( 0 , countTo , 2 )
        thisVal = packet_string[ count + 1 ]
        256 + packet_string[ count ]
        csum = csum + thisVal
...
def get_route ( hostname )
    ...
    for ttl in range ( 1 , MAX_HOPS )
        ...
        # Fill in start
        icmpHeader = recvPacket[ 2028 ]
        types , code , checksum , packetID , sequence = struct.unpack ( bbHHh , icmpHeader )
        # Fetch the icmp type from the IP packet
        # Fill in end
        if types == 11
            bytes = struct.calcsize ( d )
            timeSent = struct.unpack ( d , recvPacket[ 2828 + bytes ] )[ 0 ]
            # Fill in start
            tracelist1.append ( str ( ttl ) )
            tracelist1.append ( % .0
            fms % ((timeReceived - t)1000))
            tracelist1.append ( str ( addr[ 0 ] ) )
            tracelist1.append ( ip_to_host ( addr[ 0 ] ) )
            print ( % d % .0
            fms % s % (ttl , (timeReceived - t)1000 , addr[ 0 ]) + +ip_to_host ( addr[ 0 ] ))
            # Fill in end
            elif types == 3
            bytes = struct.calcsize ( d )
            timeSent = struct.unpack ( d , recvPacket[ 2828 + bytes ] )[ 0 ]
